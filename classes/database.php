...
class database{
    ...
    function opencon(): PDO{
        return new PDO("mysql:host=localhost; dbname=kurt-dbs", username:"root", password: "");
    }
...
function getActiveBorrowers(){
    ...
    $con = $this->opencon();
    return $con->query("SELECT borrower_id, CONCAT
    (borrower_firstname, ' ', borrower_lastname) 
    AS borrower_name 
    FROM borrowers WHERE is_active=1 
    ORDER BY borrower_name")->fetchAll();
...
function getAvailableCopies(){
    ...
    FROM
        bookcopy
    JOIN
    books on bookcopy.book_id = books.book_id 
